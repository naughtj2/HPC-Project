/**
 * @file lrm.cc
 * @brief Implementation of LRM estimators (Black–Scholes).
 */
#include "lrm.h"
namespace pricer {
    /** @brief One-sample LRM delta/vega estimators */
    std::tuple<double,double> LRM::delta_vega_european_bs(const OptionSpec& opt, RNG& rng, bool antithetic){
        ...
        auto one = [&](double Z){ 
            ...
            double payoff=(opt.type==OptionType::EuropeanPut)? std::max(K-ST,0.0): std::max(ST-K,0.0);
